...
export const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  const { summonerName, championName, tier, rank, mastery, ability, stats } =
    participant;
    ...
    <div className="w-[200px] min-h-[270px] bg-gradient-to-br from-bg-secondary/95 to-bg-primary/95 rounded-xl p-3 shadow-lg transition-all duration-slow relative border border-white/5 hover:-translate-y-1 hover:shadow-xl hover:border-primary/30 animate-slide-up">
        ...
        <div className="flex-1 flex flex-col gap-1 overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 text-[9px]">
            ...
            <span className="text-text-tertiary font-semibold">
              {formatMasteryPoints(mastery.points)}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2.5 mb-3 p-2 bg-bg-elevated/50 rounded-lg">
        ...
        <div className="flex-1 flex flex-col gap-0.5">
          ...
          <div className="text-[8px] text-text-disabled leading-tight">
            Ranked Solo/Duo
          </div>
      ...
      {showStats && stats && (
        <div className="flex gap-2 mb-3">
          <div className="flex-1 relative px-1.5 py-2 bg-bg-elevated/50 rounded-md text-center cursor-pointer transition-colors duration-base hover:bg-bg-elevated/70 group">
            <span className="block text-[8px] text-text-tertiary mb-1 uppercase tracking-wider">
              Recent Games
            </span>
            ...
            {stats.recentMatches && stats.recentMatches.length > 0 && (
              <div className="hidden group-hover:block absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-bg-primary border-2 border-primary/60 rounded-lg p-4 min-w-[240px] max-w-[300px] z-[1000] shadow-2xl animate-fade-in backdrop-blur-sm">
                ...
                <ul className="list-none m-0 p-0">
                  {stats.recentMatches.slice(0, 5).map((match, i) => (
                    <li
                      key={i}
                      className="flex justify-between items-center py-2 text-[11px] border-b border-white/10 last:border-b-0"
                    >
                      <span className="text-white font-bold flex-1">
                        {match.champion}
                      </span>
                      <span className="text-text-secondary mx-2 font-semibold">
                        {match.kda}
                      </span>
                      <span
                        className={`font-bold px-2 py-1 rounded text-[10px] ${match.win ? "bg-success/30 text-success" : "bg-error/30 text-error"}`}
                      >
                        {match.win ? "W" : "L"}
                      </span>
                    </li>
                ...
                <ul className="list-none m-0 p-0">
                  {stats.topChampions.slice(0, 3).map((champion, i) => (
                    ...
                    >
                      ...
                      <span className="text-secondary mx-2 font-bold">
                        {champion.winRate}%
                      </span>
