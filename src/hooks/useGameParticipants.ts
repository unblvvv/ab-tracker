...
import { adaptAbilityData } from "../utils/game.utils";
  ...
  const fetchParticipants = useCallback(async (): Promise<void> => {
    try {
      ...
      if (hasFetchedOnce && participants.length > 0) {
        logger.info(
          "Participants data already loaded. Stopping polling to prevent data loss.",
        );
      ...
      if (accountData?.currentGame?.participants) {
        // Adapt ability data for backward compatibility with old backend format
        const newParticipants = accountData.currentGame.participants.map(
          (participant) => ({
            ...participant,
            ability: adaptAbilityData(
              participant.ability,
      ...
      if (newErrorCount >= MAX_ERROR_COUNT) {
        const newInterval = Math.min(
          pollingInterval *
            Math.pow(BACKOFF_MULTIPLIER, newErrorCount - MAX_ERROR_COUNT),
          60000, // Max 60 seconds
        );
        setCurrentPollingInterval(newInterval);
        logger.warn(
          `Multiple errors detected (${newErrorCount}), increasing polling interval to ${newInterval}ms`,
        );
      }

      logger.error(
        `Failed to fetch participants (attempt ${newErrorCount})`,
        err,
      );
    } finally {
      setIsLoading(false);
